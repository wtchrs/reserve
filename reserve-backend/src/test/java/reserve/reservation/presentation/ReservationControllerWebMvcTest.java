...
class ReservationControllerWebMvcTest {
    ...
    void testCreateEndpoint() throws Exception {
        ...
        Mockito
            .when(reservationService.create(Mockito.eq(userId),
                    Mockito.argThat(arg -> arg.getStoreId().equals(1L)
                            && arg.getDate().equals(LocalDate.now().plusDays(7)) && arg.getHour() == 12)))
            .thenReturn(expectedReservationId);

        SignInToken signInToken = jwtProvider.generateSignInToken(TestUtils.getTokenDetails(userId));

        mockMvc
            .perform(post("/v1/reservations").header("Authorization", "Bearer " + signInToken.getAccessToken())
        ...
        Mockito.verify(reservationService, Mockito.times(1))
            .create(Mockito.eq(userId), Mockito.argThat(arg -> arg.getStoreId().equals(1L)
                    && arg.getDate().equals(LocalDate.now().plusDays(7)) && arg.getHour() == 12));
        Mockito.verify(notificationService, Mockito.times(1))
            .notifyReservation(expectedReservationId, "Reservation has been created.",
    ...
    void testGetReservationInfoEndpoint() throws Exception {
        ...
        ReservationInfoResponse expectedResponse = new ReservationInfoResponse(reservationId, storeId, "registrant",
                "username", LocalDate.now().plusDays(7), 12);
        ...
        mockMvc
            .perform(get("/v1/reservations/{reservationId}", reservationId).header("Authorization",
            ...
            .andExpectAll(status().isOk(), content().contentType("application/json"),
                    ...
                    jsonPath("$.date").value(LocalDate.now().plusDays(7).toString()), jsonPath("$.hour").value(12));
    ...
    void testGetReservationMenusEndpoint() throws Exception {
        ...
        mockMvc
            .perform(get("/v1/reservations/{reservationId}/menus", reservationId).header("Authorization",
                    "Bearer " + signInToken.getAccessToken()))
            .andExpectAll(status().isOk(), content().contentType("application/json"), jsonPath("$.count").value(3),
                    jsonPath("$.results[0].name").value("menuName1"), jsonPath("$.results[0].price").value(10000),
                    jsonPath("$.results[0].quantity").value(2), jsonPath("$.results[1].name").value("menuName2"),
                    jsonPath("$.results[1].price").value(5000), jsonPath("$.results[1].quantity").value(1),
                    jsonPath("$.results[2].name").value("menuName3"), jsonPath("$.results[2].price").value(20000),
                    jsonPath("$.results[2].quantity").value(1));
    }

    private static ReservationMenuResponse getReservationMenuResponse(Long reservationMenuId, String name, int price,
            int quantity) {
    ...
    void testSearchEndpoint() throws Exception {
        ...
        ReservationInfoResponse reservation1 = new ReservationInfoResponse(1L, 1L, "user1", "store1",
                LocalDate.now().plusDays(7), 12);
        ReservationInfoResponse reservation2 = new ReservationInfoResponse(2L, 2L, "user2", "store2",
                LocalDate.now().plusDays(7), 13);
        ReservationInfoResponse reservation3 = new ReservationInfoResponse(3L, 3L, "user3", "store3",
                LocalDate.now().plusDays(7), 14);
        ...
        Mockito.when(reservationService.search(Mockito.eq(userId), Mockito.any(), Mockito.eq(PageRequest.of(0, 20))))
            .thenReturn(ReservationInfoListResponse.from(new PageImpl<>(expectedResponse, PageRequest.of(0, 20), 3)));

        SignInToken signInToken = jwtProvider.generateSignInToken(TestUtils.getTokenDetails(userId));

        mockMvc
            .perform(get("/v1/reservations").header("Authorization", "Bearer " + signInToken.getAccessToken())
            ...
            .andExpectAll(status().isOk(), content().contentType("application/json"), jsonPath("$.count").value(3),
                    ...
                    jsonPath("$.results[0].hour").value(12), jsonPath("$.results[1].storeId").value(2L),
                    jsonPath("$.results[1].date").value(LocalDate.now().plusDays(7).toString()),
                    jsonPath("$.results[1].hour").value(13), jsonPath("$.results[2].storeId").value(3L),
    ...
    void testUpdateEndpoint() throws Exception {
        ...
        mockMvc
            .perform(put("/v1/reservations/{reservationId}", reservationId)
        ...
        Mockito.verify(reservationService, Mockito.times(1))
            .update(Mockito.eq(userId), Mockito.eq(reservationId),
                    Mockito.argThat(arg -> arg.getDate().equals(LocalDate.now().plusDays(14)) && arg.getHour() == 14));
        Mockito.verify(notificationService, Mockito.times(1))
            .notifyReservation(Mockito.eq(reservationId), Mockito.eq("Reservation has been updated."),
    ...
    void testCancelEndpoint() throws Exception {
        ...
        mockMvc
            .perform(post("/v1/reservations/{reservationId}/cancel", reservationId).header("Authorization",
                    "Bearer " + signInToken.getAccessToken()))
            .andExpect(status().isOk());

        Mockito.verify(reservationService, Mockito.times(1)).cancel(Mockito.eq(userId), Mockito.eq(reservationId));
        Mockito.verify(notificationService, Mockito.times(1))
            .notifyReservation(Mockito.eq(reservationId), Mockito.eq("Reservation has been canceled."),
